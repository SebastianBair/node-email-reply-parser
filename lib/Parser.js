...
const SIGNATURE_REGEX = [
    ...
    /^-- $/,
    /^={30,}$/,
    /^Sent from my (?:\s*\w+){1,4}$/,
    /^Von meinem (?:\s*\w+) gesendet{1,4}$/,
    /^Gesendet von (?:\s*\w+){1,4} für (?:\s*\w+){1,4}$/
];
...
const QUOTE_HEADERS_REGEX = [
    ...
    /^\s*(On(?:(?!.*Am\b|\bschrieb:)[\s\S])+schrieb:)$/m, // Am DATE, NAME <EMAIL> schrieb:
...
class Parser {
     ...
     */
    _isSignature(text) {
        for (var option of this._signatureRegex) {
            if (option.test(esrever.reverse(text))) {
                return true;
            }
        }
        return false;
